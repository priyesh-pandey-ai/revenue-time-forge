...
// Reusable component for rendering a single framework card
const FrameworkCard = ({ framework, isVisible }: { framework: typeof frameworks[0], isVisible: boolean }) => (
  <Card className={`border-border/50 bg-card p-8 md:p-12 shadow-[0_0_50px_rgba(0,0,0,0.3)] hover:shadow-[0_0_70px_rgba(139,92,246,0.2)] transition-all duration-500 ${isVisible ? 'animate-fade-in' : 'opacity-0'}`}>
    <div className="mb-6">
  ...
  return (
    <section id="frameworks" className="relative py-20 bg-gradient-to-b from-background to-muted/30 overflow-hidden">
      ...
      <div className="container relative px-6" ref={ref}>
        ...
        <Tabs defaultValue="crust" className={`w-full print:hidden transition-all duration-700 ${isVisible ? 'animate-scale-in' : 'opacity-0'}`} style={{ animationDelay: "0.2s" }}>
        ...
        {/* Print-only view: Show all frameworks when printing */}
        <div className="hidden print:block print:space-y-8">
          {frameworks.map((framework) => (
            <FrameworkCard key={framework.id} framework={framework} isVisible={true} />
          ))}
        </div>
